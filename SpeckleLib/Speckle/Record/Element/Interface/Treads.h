#ifndef SPECKLE_RECORD_ELEMENT_TREADS
#define SPECKLE_RECORD_ELEMENT_TREADS
...
namespace speckle::record::element {
	...
	/*!
	 Interface for the treads in a stair assembly
	 
	 Note that the treads use lazy loading to avoid high overheads when accessing data relevant to the parent stair only
	 */
	class Treads : public Part {
	public:
		...
		using base = Part;
			///Unique pointer
		using Unique = std::unique_ptr<Treads>;
			///Shared pointer
		using Shared = std::shared_ptr<Treads>;
			///Optional
		using Option = std::optional<Treads>;
		 ...
		 */
		size_t getTreadCount() const;
		/*!
		 Get a stair tread
		 @param index The index of the required tread
		 @return The requested tread, nullptr on error
		 */
		StairTread* getTread(size_t index) const;
	...
	private:
		class Data;
			///The tread data - mutable to support lazy loading
		mutable std::unique_ptr<Data> m_data;
	};

}

#endif	//SPECKLE_RECORD_ELEMENT_TREADS
