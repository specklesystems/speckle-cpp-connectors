#ifndef SPECKLE_RECORD_ELEMENT_STRUCTURED_STAIR
#define SPECKLE_RECORD_ELEMENT_STRUCTURED_STAIR
...
namespace speckle::record::element {
	...
	class StairStructure;
	
	/*!
	 Interface for a stair constructed o underlying structural components
	 
	 Note that the child structures use lazy loading to avoid high overheads when accessing data relevant to the parent column only
	 */
	class StructuredStair : public Part {
	public:
		...
		using base = Part;
			///Unique pointer
		using Unique = std::unique_ptr<StructuredStair>;
			///Shared pointer
		using Shared = std::shared_ptr<StructuredStair>;
			///Optional
		using Option = std::optional<StructuredStair>;
		 ...
		 */
		StructuredStair(const StructuredStair& source);
		/*!
		 Destructor
		 */
		~StructuredStair();
		...
		/*!
		 Get a column structure
		 @param index The index of the required structure
		 @return The requested structure, nullptr on error
		 */
		StairStructure* getStructure(size_t index) const;
	...
	private:
		class Data;
			///The structure data - mutable to support lazy loading
		mutable std::unique_ptr<Data> m_data;
	};

}

#endif	//SPECKLE_RECORD_ELEMENT_STRUCTURED_STAIR
